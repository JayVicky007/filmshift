  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({
        inline: false, 
        HTMLAttributes: {
          // 🚀 UNIQUE FILMSHIFT FRAME: Non-invasive golden accent border, deep shadow, and subtle inner padding alignment
          class: "rounded-2xl border border-accent/20 bg-surface/50 p-1.5 my-6 max-w-full h-auto mx-auto shadow-[0_12px_40px_rgba(0,0,0,0.25)] block transition-all hover:border-accent/50 pointer-events-auto cursor-grab active:cursor-grabbing",
        },
      }),
      Placeholder.configure({ placeholder: "Share your cinematic thoughts..." }),
    ],
...
// ==========================================================================
// 🚀 PHASE 2 OF 3: src/components/WritePostForm.tsx (Form Fields & Headings)
// ==========================================================================
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!editor) return;
...
{/* 🚀 PHASE 3 OF 3: src/components/WritePostForm.tsx (Direct Injection & Right Handle) */}
            ...
            {/* 🎯 DEVICE IMAGE UPLOADING UTILITY WITH 5MB PLATFORM RESTRICTION */}
            <input 
              type="file" 
              ref={fileInputRef} 
              accept="image/*" 
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => {
                ...
                // PLATFORM PROTECTOR RULE: Strictly limit uploaded content sizes to 5MB max
                ...
                reader.onload = () => {
                  ...
                  // 🚀 INSTANT DIRECT AUTO-RESIZE & INJECT PIPELINE: Bypasses cropping modal completely!
                  if (editor && imageSrc) {
